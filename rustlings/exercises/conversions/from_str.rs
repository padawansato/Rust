...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Person, Self::Err> {
        ...
        } else if v[0].is_empty() || v[0].is_empty() {
            Err("Empty name or age field".to_string())
        } else {
            let age = match v[1].parse::<usize>() {
                Ok(age) => return Ok(Person{name: String::from(v[0]),age: age}),
