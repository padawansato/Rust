...
イテレータ3.rs
これは、他のほとんどの練習問題よりも大きなものです。あなたならできます。
ここにあなたの使命があります、それを受け入れることを選択してください。
1. 最初の4つのテストがパスするようにdivide関数を完成させてください。
2. result_with_list と list_of_results 関数を完成させ、残りのテストを通過させる。
   list_of_results 関数を完成させ、残りのテストを通過させる。
ヒントを得るために `rustlings hint iterators3` を実行してください。
